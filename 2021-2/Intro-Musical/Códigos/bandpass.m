...
% Constrói o filtro passa-banda
dw = 1/delta;
frequencia = 10; % exemplo de frequencia central que define o filtro
sigma = 2;  % o desvio padrão deve ser ajustado para cortar as frequências desejadas!
for n = [1:dw]
     filtro(n) = exp((-1)*(((n-frequencia)*(n-frequencia))/(2*sigma*sigma)));
endfor
filtro = filtro ./ filtro(1);

hold on; 
figure(3);
plot([0:(H*f0)],filtro(1:(H*f0)+1),'LineWidth',2,'Color','b'); % intervalo ajustado para exibição apenas
xlabel('Frequência (Hz)');
ylabel('Magnitude');
hold off;

for k = [1:dw]
  ft_sinal_filtrado =   ft_sinal .* filtro;
endfor
